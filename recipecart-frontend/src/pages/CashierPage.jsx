...
export default function CashierPage() {
  ...
  const handleCancelOrder = (orderId, buyerName) => {
    const confirmed = window.confirm(
      `Are you sure you want to cancel the order from ${buyerName}?`
    );
    if (confirmed) {
      setOrders(prevOrders => prevOrders.filter(o => o.id !== orderId));
      setActiveDropdown(null);
      setSuccessMessage(`Order from ${buyerName} has been canceled successfully.`);
              ...
              <td className="centered-cell">
                <div className="product-images-wrapper">
                  {order.products.map((p, idx) => (
                    <img key={idx} src={p.picture} alt={p.name} className="item-image" />
                  ))}
                </div>
              </td>
                  ...
                  {activeDropdown === order.id && (
                    <div className="dropdown-menu">
                      <button
                        ...
                        onClick={() => handleCancelOrder(order.id, order.buyer.name)}
                      >
                        Cancel Order
                      </button>
